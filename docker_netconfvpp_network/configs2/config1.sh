...
$VPPCTL -s $CLISOCK1 create memif socket id 0 filename /shared/vpp/memif0.sock
$VPPCTL -s $CLISOCK1 create interface memif id 0 socket-id 0 master
#$VPPCTL -s $CLISOCK1 create host-interface name eth0
sleep 1
$VPPCTL -s $CLISOCK1 set interface ip address memif0/0 2000::4/64
sleep 1
...
$VPPCTL -s $CLISOCK1 ip route add b001::/64 via 2000::5 memif0/0
